...
#################### configure lats/lons of interest ###########################
api_key = "" # api key for openweathermap.org

lat1 = 45 # positive number means North, negative number means South
lat2 = 75 # positive number means North, negative number means South

lon1 = -20 # positive number means East, negative number means West
lon2 = 10 # positive number means East, negative number means West
...
increment = 1 # to increase lat/lon co-ordinates by this amount
################################################################################
...
for iii in range(len(X_dat)):
    X = np.append(X, X_dat[iii])
    Y = np.append(Y, Y_dat[iii])
    Z = np.append(Z, Z_dat[iii])
    
# create x-y points to be used in heatmap
xi = np.linspace((X.min()), (X.max()), len(dff['lon']))
yi = np.linspace((Y.min()), (Y.max()), len(dff['lon']))

# Interpolate for plotting
zi = griddata((X, Y), Z, (xi[None,:], yi[:,None]), method='cubic')

# graph the data
base_map_plot(sdata=1,bar=True,
              title="Wind speed UK 06/02/2023")
